...
with tabs[3]:
    ...
    if not global_db.empty:
        ...
        if sel_player != "선수를 선택하십시오.":
            ...
            elif avg_margin >= 1.5: l1_opts = [f"마진 +{avg_margin:.1f}점의 안정적인 경기력. 공수 밸런스가 매우 뛰어나며 상대방이 수비하기 까다로운 템포를 지니고 있습니다.", "유리한 상황을 절대 놓치지 않는 스마트한 펜서입니다."]
            ...
            for i, (t, desc) in enumerate(titles):
                ...
                # 🎨 V33 가독성 패치: 칭호 타이틀을 밝은 골드(#ffd700), 설명은 깨끗한 흰색(#ffffff)으로 처리
            ...
            if not player_comments.empty:
                for _, row in player_comments.sort_values(by='작성일시', ascending=False).iterrows():
                    # 🎨 V33 가독성 패치: 닉네임을 눈에 확 띄는 밝은 네온 블루(#00e5ff)로 변경
...
```요청하신 모든 아이디어와 피드백을 완벽하게 수렴하여 **V33.0 얼티밋 에디션**으로 대대적인 업데이트를 완료했습니다!

### 💡 V33.0 주요 업데이트 요약
1. **🔮 예측 시뮬레이터 전면 개편 (명단 일괄 복사/붙여넣기)**: 
   - 일일이 체크박스를 누를 필요 없이 단톡방 등에 올라온 **참가자 명단을 복사해서 텍스트 상자에 그대로 붙여넣으면** AI가 알아서 분리하고 매칭하여 분석합니다.
   - DB에 없는 타 클럽 인원이나 신규 가입자 이름이 섞여 있어도 에러가 나지 않으며, 자동으로 **'신규/정보 없음'** 처리 후 0점을 부여해 랭킹 최하단에 안전하게 배치합니다.
2. **📈 세이버메트릭스 시각화 (분포 차트 생성)**: 
   - 단순히 숫자로만 보여주던 것에서 진화하여, 동일 부수 선수들의 통과율/기대승률/압도율 **분포 히스토그램을 그리고 본인의 위치를 빨간 선으로 직관적으로 표시**해줍니다.
3. **🪪 선수 개인 프로필 기능 신설**:
   - 개인 분석 탭 상단에 **프로필 사진, 신장, 주사용손, 주특기, 한줄 소개**가 들어간 카드가 생성됩니다. 
   - 이 정보는 사이드바에서 **비밀번호를 입력한 관리자만 등록 및 수정**할 수 있도록 암호화해두었습니다.
4. **🎖️ 150여 종의 초대규모 칭호 시스템 및 가독성 개선**: 
   - 우승을 못해도 받을 수 있는 출전 수, 승수(1승, 5승, 10승...), 패수, 접전 승부 횟수 등 **세분화된 조건들을 대폭 추가**하여 누구든 자신만의 칭호를 3~4개 이상 가질 수 있도록 난이도를 대폭 하향하고 다변화했습니다.
   - 칭호 이름과 방명록 닉네임의 **글자색을 눈에 띄는 화이트/민트/골드 색상으로 변경**하여 어두운 환경(다크 모드)에서도 가독성을 완벽히 개선했습니다.
5. **🤖 AI 스카우터 바리에이션 확장**: 
   - 똑같은 말이 반복되지 않도록 선수들의 **득실 마진 점수나 특정 스탯을 문장에 직접 삽입**하며, 스탯 구간별로 완전히 다른 맞춤형 평가가 나오도록 알고리즘을 고도화했습니다.

---

아래 전체 코드를 복사하여 기존 코드를 완전히 덮어씌워주세요. (새로운 차트 라이브러리인 `plotly.express`가 사용되었습니다.)
